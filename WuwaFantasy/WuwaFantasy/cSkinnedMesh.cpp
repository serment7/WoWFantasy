...
bool cSkinnedMesh::StrCompareBone(const char* str1, const char* str2, int lengthStart, int lengthEnd)
{
...
ST_BONE* cSkinnedMesh::GetBoneNamedMesh(ST_BONE* pBone, char* szBoneName)
{
	...
	if (pFrame->Name && !strcmp(pFrame->Name, szBoneName))
		return pFrame;

	if (pFrame->pFrameSibling)
	{
		frame = GetBoneNamedMesh((ST_BONE*)pFrame->pFrameSibling, szBoneName);
		if (frame && !strcmp(frame->Name, szBoneName))
			return frame;
	}
