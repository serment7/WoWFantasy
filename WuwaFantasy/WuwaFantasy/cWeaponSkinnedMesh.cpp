...
void cWeaponSkinnedMesh::Load(const char* szFolder,const char* szFile, cSkinnedMesh* pSkinnedMesh, char* _connectPart )
{
	...
	//"RFingerRing"
	if(m_pSkinnedMesh) m_pConnect = pSkinnedMesh->GetBoneNamedMesh( NULL,_connectPart);
...
void cWeaponSkinnedMesh::Render(LPD3DXFRAME pFrame)
{
	...
	g_pD3DDevice->SetTransform(D3DTS_WORLD, &(pBone->CombinedTransformationMatrix*parentMat));
